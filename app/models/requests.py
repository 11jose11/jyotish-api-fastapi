...
class EphemerisRequest(BaseModel):
    ...
    def validate_timestamp_combination(self):
        ...
        when_utc = self.when_utc
        when_local = self.when_local
...
class CalendarRequest(BaseModel):
    ...
    anchor: str = Field(
        default="sunrise",
        pattern="^(sunrise|midnight|noon|custom)$",
        description="Anchor time"
    )
    custom_time: Optional[str] = Field(None, pattern="^([01]?[0-9]|2[0-3]):[0-5][0-9]$")
    format: str = Field(default="compact", pattern="^(compact|detailed)$")
    planets: str = Field(
        default="Sun,Moon,Mercury,Venus,Mars,Jupiter,Saturn,Rahu,Ketu"
    )
    units: str = Field(default="both", pattern="^(decimal|dms|both)$")
...
class MotionRequest(BaseModel):
    ...
    mode: str = Field(default="classic", pattern="^(classic|adaptive)$")
...
class YogaDetectionRequest(BaseModel):
    ...
    granularity: str = Field(default="day", pattern="^(day|intervals)$")
...
class PlaceAutocompleteRequest(BaseModel):
    ...
    language: str = Field(default="en", pattern="^[a-z]{2}$", description="Language code")
